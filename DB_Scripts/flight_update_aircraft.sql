...
BEGIN
  ...
  UPDATE aircraft_spotted SET last_flight_number=_flight_num
          WHERE icao_hex=_icao_hex;
  END IF;
  -- check and update lat/long
  IF _lat != 0 AND _long != 0 THEN
   UPDATE aircraft_spotted SET last_position=st_makepoint(_long, _lat)
      WHERE icao_hex=_icao_hex;
  END IF;
   -- check and update avg speed
  IF _speed > 0 THEN
    UPDATE aircraft_spotted SET avg_speed=((_speed+avg_speed)/2)
      WHERE icao_hex=_icao_hex;
  END IF;

  -- check and update avg alt
  IF _alt > 0 THEN
    UPDATE aircraft_spotted SET avg_alt=((_alt+avg_alt)/2)
      WHERE icao_hex=_icao_hex;
  END IF;
  return 1;
