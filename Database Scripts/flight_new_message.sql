...
BEGIN
	...
	IF NOT exists(SELECT 1 FROM aircraft_spotted WHERE aircraft_spotted.icao_hex = _icao_hex) THEN
		...
		INSERT INTO aircraft_spotted VALUES (_icao_hex,
							newuuid, _flight_num, now(), _alt, _speed, 
							st_makepoint(_long, _lat), 1, -- messages recieved
							1 -- total sessions
							);
		perform flight_new_flight(newuuid, _icao_hex, _flight_num, _alt, _speed, _heading, _signal_strength, _lat, _long, _mode, _sqk);
	ELSIF exists(SELECT 1 from flights_seen WHERE flights_seen.icao_hex = _icao_hex AND final_time=(SELECT max(final_time) FROM flights_seen WHERE icao_hex=_icao_hex)) AND
							(age(now(), final_time) < '10 minutes'::interval) from flights_seen where final_time = (SELECT max(final_time)
								FROM flights_seen WHERE icao_hex=_icao_hex)THEN
		-- this message is part of a current flight
		perform flight_update_flight(_icao_hex, _flight_num, _alt, _speed, _heading, _signal_strength, _lat, _long,_mode, _sqk);
		perform flight_update_aircraft(_icao_hex, _flight_num, _alt, _speed, _lat, _long);
	...
	ELSE 
		...
		perform flight_update_aircraft(_icao_hex, _flight_num, _alt, _speed, _lat, _long);
		perform flight_new_flight(newuuid, _icao_hex, _flight_num, _alt, _speed, _heading, _signal_strength, _lat, _long, _mode, _sqk, _stationID);
	END IF;
	IF _alt=0 AND _speed=0 AND _heading=0 AND _lat=0.0 AND _long=0.0 THEN
		...
		return 1;
	END IF;
	perform flight_insert_message(_icao_hex, _alt, _speed, _heading, _signal_strength, _lat, _long, _stationID);
	return 1;
